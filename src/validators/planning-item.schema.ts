 ...
 * updated. The nullable columns (`description`, `listId`, `priorityId`,
 * `dueAt`) additionally accept an explicit `null` so the client can CLEAR
 * them (e.g. remove a due date or detach an item from its list); `null` on
 * a nullable field is a deliberate "unset", distinct from omitting the key.
 * `itemTypeId` and `statusId` are required columns, so they may be changed
 * but never cleared.
 */
